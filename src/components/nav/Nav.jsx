...
const Nav = ({ getTotalItems, handleAddToCart, handleRemoveFromCart, cartItems, }) => {
  ...
  return (
    <div className="font-poppins w-full h-24 bg-ochre flex justify-between">
      ...
      <div className={isAuthenticated ? "w-1/3 flex justify-around items-center mr-8" : "w-1/6 flex justify-beetwen items-center mr-8"}>
        {
          isAuthenticated ? (
            <div className="flex items-center">
              ...
              <img src={user.picture} alt='logo' className="w-10 rounded-full mr-3" />
              <button onClick={() => logout({ returnTo: window.location.origin })} className="flex items-center justify-center w-38 mr-3 px-4 py-2 space-x-3 text-sm text-center bg-darkGreen text-isabelline transition-colors duration-200 transform dark:text-gray-300 dark:border-gray-300 hover:bg-gray-600 dark:hover:bg-gray-700 rounded-md">
                Cerrar Sesión
              </button>
            </div>
          ) : (
